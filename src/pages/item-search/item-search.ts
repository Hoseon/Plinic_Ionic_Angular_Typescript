...
import { CommunityWritePage } from '../community//community-write/community-write';
...
export class ItemSearchPage {
  ...
  constructor(
    ...
    this.isNewMode = this.navParams.get('isNew');
  ...
  async ionViewWillEnter() {
    ...
    // await this.loadProduct(this.datamode);
    this.getHashTags();
  ...
  onCancel(event) {
    console.log("onCancel : " + event.value);
  ...
  onEnterKeyDown(event) {
    ...
    this.images.productLoad(this.keyworkd, this.scrollPage).subscribe(data => {
      // this.loading2.dismiss();
      this.productData = (data);
      console.log(data);
      // console.log(JSON.stringify(data));
    }, error => {
      console.error("화장품 데이터 불러오기 에러 발생 : " + error);
    })
 ...
 public saveMyProduct(product) {
   this.showLoading();
   if(this.isNewMode==='new') {
    console.log("product " + JSON.stringify(product));
    this.auth.saveMyProduct(this.userData.email, product, this.mode).subscribe(data=> {
      console.log("화장품 저장 데이터 : " + JSON.stringify(data));
      ...
      this.loading.dismiss();
      console.log("화장품 데이터 등록 오류 : " + error);
    });
   } else {
    console.log("product " + JSON.stringify(product));
    this.auth.delAndSaveMyProduct(this.userData.email, product, this.mode).subscribe(data=> {
      console.log("delAndSaveMyProduct 화장품 저장 데이터 : " + JSON.stringify(data));
      this.loading.dismiss();
      if(data){
        this.nav.pop();
      }
      ...
      this.loading.dismiss()
      console.log("화장품 데이터 지우기 에러 :" + error);
    });
   }
 }
...
//  public searchProduct(mode, search) {
//   this.images.productLoad(search).subscribe(data => {
//     if(mode === "date"){
//       this.productData = data;
//       console.log(JSON.stringify(data));
//       this.beautyNoteData = data.sort((a, b) => {
