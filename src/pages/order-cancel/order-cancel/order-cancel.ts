...
export class OrderCancelPage {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.updateCancelOrder('chs0131@hanmail.net', this.reasonType, this.reasonDesc);
  ...
  async updateCancelOrder(email, reasonType, reasonDesc) {
    this.presentLoadingCustom();
    if (email && reasonType && reasonDesc.length > 2) {
      await this.authService.updateCancelOrders(email, reasonType, reasonDesc, this.detailData._id).subscribe(data => { //서버처리 완료
        this.billCancelCompleted();
        this.loading.dismiss();
      }, error => {
        this.errorData();
