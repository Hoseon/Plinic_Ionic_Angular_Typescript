...
/**
 * Generated class for the OrderChangePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-order-change',
  templateUrl: 'order-change.html',
  })
  

export class OrderChangePage {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.updateChageOrder('chs0131@hanmail.net', this.reasonType, this.reasonDesc);
  ...
  async updateChageOrder(email, reasonType, reasonDesc) {
    this.presentLoadingCustom();
    if (email && reasonType && reasonDesc.length > 2) {
      await this.authService.updateChangeOrders(email, reasonType, reasonDesc, this.detailData._id).subscribe(data => { //서버처리 완료
        this.billCancelCompleted();
        this.loading.dismiss();
      }, error => {
        this.errorData();
