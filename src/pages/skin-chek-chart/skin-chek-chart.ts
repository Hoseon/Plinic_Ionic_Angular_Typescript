...
export class SkinChekChartPage {
  ...
  getWeather() {
    ...
    String(timeN).length === 1
      ? (time = "0" + String(timeN) + "00")
      : (time = String(timeN) + "00");
    ...
    this.images.getWeather(today, time).subscribe((data) => {
      ...
      if (this.sky[0] === "1") {
        this.skyResult = "맑음";
        this.skyColor = "#4c68e0";
      } else if (this.sky[0] === "3") {
        this.skyResult = "구름많음";
        this.skyColor = "#ff3939";
      } else {
        this.skyResult = "흐림";
        this.skyColor = "#ac0000";
      }
      ...
      if (Number(this.rehResult) > 45) {
        ...
        Number(this.rehResult) <= 55 ||
        Number(this.rehResult) >= 100
      ) {
  ...
  getMiseDescription(pm10: number) {
  ...
  poreSize() {
    this.navCtrl
      .push(PoreSizePage, {
        skinAnalyData: this.skinAnalyData,
    ...
    this.auth.getSkinAnaly(this.userData.email).subscribe(
      (data) => {
        if (data) {
          ...
          this.skinAnalyPoreSize = Math.floor(
            this.skinAnalyData.cheek[this.skinAnalyData.cheek.length - 1]
              .pore[0].average_pore
          );
          ...
          this.skinAnalyPoreSizeAvg = Math.floor(
            Number(sizeSum / this.skinAnalyData.cheek.length)
          ); //전체 모공사이즈 평균

          this.skinAnalyPoreCompareAvg = Math.floor(
            this.skinAnalyPoreSize - this.skinAnalyPoreSizeAvg
          );
          this.skinAnalyPoreCompareAvg > 0
            ? (this.skinAnalyPoreCompareAvg =
                "+" + String(this.skinAnalyPoreCompareAvg))
            : this.skinAnalyPoreCompareAvg;
          ...
          this.skinAnalyPoreBeforeSizeAvg = Math.floor(
            Number(beforeSum / (this.skinAnalyData.cheek.length - 1))
          ); //직전까지 모공사이즈 평균
          this.skinAnalyPoreBeforeSizeAvg >= 0
            ? this.skinAnalyPoreBeforeSizeAvg
            : (this.skinAnalyPoreBeforeSizeAvg = "--");
          if (this.skinAnalyPoreBeforeSizeAvg >= 0) {
            this.skinAnalyPoreCompareSize =
              this.skinAnalyPoreSizeAvg - this.skinAnalyPoreBeforeSizeAvg;
            this.skinAnalyPoreCompareSize > 0
              ? (this.skinAnalyPoreCompareSize =
                  "+" + String(this.skinAnalyPoreCompareSize))
              : this.skinAnalyPoreCompareSize;
          }

          //현재 모공 갯수 총합

          this.skinAnalyPoreOneCount = this.skinAnalyData.cheek[
            this.skinAnalyData.cheek.length - 1
          ].pore[0].pore_count;
          ...
          this.skinAnalyPoreCount = Math.floor(
            Number(poreCountSum / this.skinAnalyData.cheek.length)
          ); //전체 모공사이즈 평균
          ...
          this.skinAnalyPoreBeforeCount = Math.floor(
            Number(poreBeforCountSum / (this.skinAnalyData.cheek.length - 1))
          ); //전체 모공사이즈 평균
          this.skinAnalyPoreBeforeCount >= 0
            ? this.skinAnalyPoreBeforeCount
            : (this.skinAnalyPoreBeforeCount = "--");

          //모공 직전까지 평균
          if (this.skinAnalyPoreBeforeCount >= 0) {
            this.skinAnalyPoreCompareCount =
              this.skinAnalyPoreOneCount - this.skinAnalyPoreBeforeCount;
            this.skinAnalyPoreCompareCount > 0
              ? (this.skinAnalyPoreCompareCount =
          ...
          if (
            this.skinAnalyData.cheek[this.skinAnalyData.cheek.length - 1].diff
              .length > 0
          ) {
            this.skinCleanScore = Math.floor(
              this.skinAnalyData.cheek[this.skinAnalyData.cheek.length - 1]
                .diff[0].value
            );
            ...
            this.skinCleanScore > 0
              ? (this.skinCleanScore = "+" + String(this.skinCleanScore))
              : this.skinCleanScore;
          } else {
            this.faceText = "--";
            this.faceImgUrl = "assets/img/skin-chek-chart/face_good.png";
          ...
          //문진표 결과
          if (
            this.skinAnalyData.munjin[this.skinAnalyData.cheek.length - 1]
              .sleep === 11
          ) {
          ...
          this.munjinCreateAt = this.skinAnalyData.munjin[
            this.skinAnalyData.cheek.length - 1
          ].created_at;
  ...
  getAgeRange() {
    ...
    age =
      ...
      Number(this.userData.birthday.substr(0, 4)) +
      1;
    return String(age).substr(0, 1) + "0";
  }

  getAvgSkinPore(ageRange) {
    this.images.getAvgPore(ageRange, this.userData.gender).subscribe(
      (data) => {
        ...
        this.skinAnalyAvgCompare = Math.floor(
          Number(this.skinAnalyPoreSizeAvg) - Number(this.avgCheekPoreSize)
        );
        this.skinAnalyAvgCompare >= 0
          ? this.skinAnalyAvgCompare
          : (this.skinAnalyAvgCompare = "--");
        if (this.skinAnalyAvgCompare > 0) {
          this.skinAnalyAvgCompare > 0
            ? (this.skinAnalyAvgCompare =
                "+" + String(this.skinAnalyAvgCompare))
            : this.skinAnalyAvgCompare;
        }

        this.skinAnalyAvgComparePoreCount = Math.floor(
          Number(this.skinAnalyPoreCount) - Number(this.avgCheekPoreCount)
        );
        this.skinAnalyAvgComparePoreCount >= 0
  ...
  public loadMyMainProduct() {
    this.images.myMainProductLoad(this.userData.email).subscribe(
      (data) => {
        if (data != "") {
  ...
  getHumidityDesc(humidity : number) {
    if (Number(humidity) > 45) {
      this.rehTextResult = "낮음";
      this.rehColor = "#4c68e0";
    } else if (Number(humidity) <= 45 || Number(this.rehResult) > 55) {
      this.rehTextResult = "보통";
      this.rehColor = "#0ca28f";
    } else if (
      Number(humidity) <= 55 ||
  ...
  checkLoadingDismiss(load1, load2) {
